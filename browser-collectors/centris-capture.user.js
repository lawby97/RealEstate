// ==UserScript==
// @name         Quebec Capture - Centris
// @namespace    https://localhost:3000
// @version      0.1
// @description  Browser-assisted Centris capture for the local Quebec ingest pipeline.
// @match        https://www.centris.ca/*
...
(function () {
  ...
  function looksLikeListingPayload(payload) {
    ...
    return Boolean(payload.Results || payload.results || payload.listings || payload.items || payload.data);
  }
  ...
  function captureJson(payload, url) {
    if (!looksLikeListingPayload(payload)) return;
    lastPayload = payload;
    lastPayloadUrl = url;
    lastPayloadType = Array.isArray(payload) ? "search_results" : (payload.Results || payload.results || payload.listings || payload.items || payload.data) ? "search_results" : "listing_detail";
    updateStatus(`Captured JSON from ${url}`);
  }
  ...
  XMLHttpRequest.prototype.send = function () {
    this.addEventListener("load", function () {
      try {
        ...
        captureJson(payload, this.__captureUrl || location.href);
      } catch (_) {}
    });
  ...
  panel.innerHTML = `
    <div style="font-weight:700;margin-bottom:8px">Centris capture</div>
